...
class App extends Component {
  ...
  getResults = () => {
    ...
    console.clear();
    arr.map(item => {
      console.log(`%c Position ${item.id}`, 'color: #5d38ec; font-weight: 800; font-size: 20px; margin-top: 40px');
      if (item.items.length) {
        console.table(item.items);
      } else {
        console.log(`%c Nothing added to this position 😥`, 'font-weight: 600; font-size: 16px')
      }
    })
  }
